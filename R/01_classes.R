...
generate_psychonetrics_samplestats <- setClass("psychonetrics_samplestats",  slots = c(
  ...
  weights = "list", # Per-group normalized sampling weights (each sums to n_g); empty list when no sampling weights are used. Read with .hasSlot guards for objects saved before 0.16.5.
...
generate_psychonetrics <- setClass("psychonetrics", slots = c(
  ...
  equal = "character", # Deprecated in 0.15.3 as canonical source of equality constraints: the parameters table (shared par indices) is authoritative. @equal is still maintained by groupequal()/groupfree() for backwards compatibility with model reconstruction code (bootstrap, changedata).
  objective = "numeric",
  information = "matrix",
...
prototype = list(
  ...
  parameters = data.frame(
    ...
    epc_free = numeric(0),
    mi_free_joint = numeric(0), # Joint score-test statistic
    pmi_free_joint = numeric(0), # p-value of the joint score test
    df_free_joint = numeric(0), # Degrees of freedom of the joint score test
    ...
    epc_equal = numeric(0), # Expected parameter change constraining groups to be equal
    minimum = numeric(0),
    maximum = numeric(0),
