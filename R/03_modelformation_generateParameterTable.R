...
generateParameterTable <- function(x, mat, op, curMaxPar, symmetrical = FALSE, sampletable, rownames, colnames, rowid, colid, sparse = FALSE, posdef = FALSE, diag0=FALSE, diagonal = FALSE,
  ...
  if (length(est) > 0){
    ...
    partable <- data.frame(
      ...
      epc_free = NA,
      mi_free_joint = NA, # Joint score-test statistic for releasing the full equality constraint (G-1 df)
      pmi_free_joint = NA, # p-value of the joint score test
      df_free_joint = NA, # Degrees of freedom of the joint score test
    ...
    partable <-data.frame(
      ...
      pmi_free_joint = numeric(0),
      df_free_joint = numeric(0),
