...
identify <- function(x){
  if (x@model == "ggm" | x@model == "precision"| x@model == "gvar" | x@model == "varcov" | x@model == "cholesky" | x@model == "var1" | x@model == "panelvar" | x@model == "panelvar1" | x@model == "ml_var1" | x@model == "meta_varcov" | x@model == "meta_var1"){
    # Nothing to do..
    return(x)
  }
