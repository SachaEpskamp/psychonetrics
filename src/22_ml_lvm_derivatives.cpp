...
// TWO-LEVEL (sufficient statistics ML) GROUP JACOBIAN //
// Jacobian of phi = [mu; vech(Sigma_within); vech(Sigma_between)] with
// respect to the model parameters, for the sufficient-statistics two-level
// ML estimator (estimator = "ML"). C++ twin of d_phi_theta_ml_lvm2L_group
// (R/22_ml_lvm_derivatives.R): the mean rows are identical to the wide-format
...
// [[Rcpp::export]]
arma::mat d_phi_theta_ml_lvm2L_group_cpp(
    const Rcpp::List& grouplist
){
  // Things I need now:
  std::string within_latent  = grouplist["within_latent"];
  ...
  arma::vec sigInds_within = seq_len_inds(meanInds(1) + 1, nVar*(nVar+1)/2);
  arma::vec sigInds_between = seq_len_inds(sigInds_within(1) + 1, nVar*(nVar+1)/2);
  ...
  // Fill mean to between-subject beta part (mu depends on beta_between
  // whenever nu_eta != 0; within-cluster latents have mean 0, so beta_within
  // needs no mean term):
  Jac.submat(meanInds(0),beta_between_inds(0),meanInds(1),beta_between_inds(1)) = d_mu_beta_lvm_cpp(
    grouplist["nu_eta"], grouplist["lambda"], grouplist["tBetakronBeta_between"]
  );
  ...
  // Lambda part:
  Jac.submat(sigInds_within(0),lambda_inds(0),sigInds_within(1),lambda_inds(1)) =
    d_sigma_lambda_lvm_cpp(grouplist["L_y"],grouplist["Lambda_BetaStar_within"],grouplist["Betasta_sigmaZeta_within"],grouplist["I_y"], grouplist["C_y_eta"]);

  // Beta part:
  Jac.submat(sigInds_within(0),beta_within_inds(0),sigInds_within(1),beta_within_inds(1)) = d_sigma_beta_lvm_cpp(
    grouplist["L_y"], grouplist["lambda"], grouplist["Betasta_sigmaZeta_within"], grouplist["C_eta_eta"], grouplist["I_eta"], grouplist["tBetakronBeta_within"]);

  // Fill sigma_zeta_within part:
  Jac.submat(sigInds_within(0),sigma_zeta_within_inds(0),sigInds_within(1),sigma_zeta_within_inds(1)) = d_sigma_sigma_zeta_lvm_cpp(
    grouplist["L_y"],grouplist["Lambda_BetaStar_within"],grouplist["D_eta"]) * aug_within_latent;

  // Fill sigma_epsilon_within part:
  Jac.submat(sigInds_within(0),sigma_epsilon_within_inds(0),sigInds_within(1),sigma_epsilon_within_inds(1)) =
    aug_within_residual;

  // vech(Sigma_between) part: the BETWEEN summands of the wide composition //

  // Lambda part:
  Jac.submat(sigInds_between(0),lambda_inds(0),sigInds_between(1),lambda_inds(1)) =
    d_sigma_lambda_lvm_cpp(grouplist["L_y"],grouplist["Lambda_BetaStar_between"],grouplist["Betasta_sigmaZeta_between"],grouplist["I_y"], grouplist["C_y_eta"]);

  // Beta part:
  Jac.submat(sigInds_between(0),beta_between_inds(0),sigInds_between(1),beta_between_inds(1)) = d_sigma_beta_lvm_cpp(
    grouplist["L_y"], grouplist["lambda"], grouplist["Betasta_sigmaZeta_between"], grouplist["C_eta_eta"], grouplist["I_eta"], grouplist["tBetakronBeta_between"]);

  // Fill sigma_zeta_between part:
  Jac.submat(sigInds_between(0),sigma_zeta_between_inds(0),sigInds_between(1),sigma_zeta_between_inds(1)) = d_sigma_sigma_zeta_lvm_cpp(
    grouplist["L_y"],grouplist["Lambda_BetaStar_between"],grouplist["D_eta"]) * aug_between_latent;

  // Fill sigma_epsilon_between part:
  Jac.submat(sigInds_between(0),sigma_epsilon_between_inds(0),sigInds_between(1),sigma_epsilon_between_inds(1)) =
    aug_between_residual;

  // Return (no P permutation):
  return(Jac);
}
...
arma::mat d_phi_theta_ml_lvm_cpp(
  ...
  // wide-format parameters (twin of the dispatch in d_phi_theta_ml_lvm):
  bool twolevel_ML = false;
  ...
  for (int i=0; i<nGroup;i++){
    arma::mat groupgrad;
    if (twolevel_ML){
      groupgrad = d_phi_theta_ml_lvm2L_group_cpp(groupmodels[i]);
    } else {
      groupgrad = d_phi_theta_ml_lvm_group_cpp(groupmodels[i]);
    }
